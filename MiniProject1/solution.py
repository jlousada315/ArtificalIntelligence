...
class ASARProblem(Problem):
            ...
            if state.get_values()[n] is None:
                ...
                for k in range(len(self.leg)):
                    if state.adict.get(self.air_plane[n])[-2] == self.leg[k].get_air1():
                        ...
                        if self.leg[k].__contains__(state.get_trips()) is False and time_limit is True and int(
                                b_time) > int(self.get_otime_string(self.leg[k].get_air2())):
                            possible_actions.append(info)
    ...
    def goal_test(self, state):
        ...
        keys = state.get_keys()
        flag = False
        if len(self.leg) == len(state.get_trips()):
            for i in keys:
    ...
    def heuristic(self, n):
        ...
        legs = []
        for i in self.leg:
            if i.__contains__(n.state.get_trips()) is False:
                legs.append(i)
    ...
    def load(self, file):
        ...
        self.initial = State({})
        for i in range(len(self.air_plane)):
            self.initial.set_key(self.air_plane[i])
    ...
    def save(self, file, state):
        ...
        for x, y in tup.items():
            ...
            for i in range(len(y)):
                ...
                self.profit += self.get_profit_leg(y[i][1], y[i][2], x.get_model())
            text += '\n'
        print('s', self.nr)
        file.write(text + '\r')
        file.write('P' + ' ' + str(self.profit))

    def get_max_profit(self, max_profit=0):
        """Gets max limit for total gained profit"""
...
class Leg:
    ...
    def get_cost(self, profit):
        """" Cost is given by the difference between the max possible profit
                    in a leg and the airplane's associated profit"""
...
class AirRotation:
    ...
    def __init__(self, model, rot_t):
        """" Creates aircraft class, with associated rotation time"""
...
class Airplane:
    ...
    def __init__(self, rcode, model):
        """" Creates each Airplane, specifying its class, registration code and rotation time"""
...
class State:
    """ State is a dictionaire: Key is the airplane tag, and the value an array containing the current airport and the current time
    """
...
def time_sum(a, b):
    """" Sums times written in a hhmm format"""
